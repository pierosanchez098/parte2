...
if ($conn->connect_error) {
    echo json_encode(["error" => "Error de conexión a la BD", "estudis" => [], "foto_carnet" => null]);
    exit;
}
...
if (empty($token) || empty($dni_persona)) {
    echo json_encode(["error" => "Faltan token o dni_persona", "estudis" => [], "foto_carnet" => null, "expired" => true]);
    exit;
}
...
if (!$resultado['valido']) {
    echo json_encode(["error" => "Sesión expirada o inválida", "estudis" => [], "foto_carnet" => null, "expired" => true]);
    $conn->close();
    exit;
}
